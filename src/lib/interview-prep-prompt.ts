...
const categoryConfigs: Record<CategoryKey, CategoryConfig> = {
  hr: {
    count: "6-8",
    maxTokens: 2000,
    instructions: `HR screener questions asked before any technical round. Cover ALL of these angles:
- 自我介绍（请用1-2分钟介绍一下你自己）
- 求职动机（为什么离职？为什么投我们公司？）
- 薪资期望（期望薪资范围是多少？）
- 优缺点（你最大的优点和缺点各是什么？）
...
- 入职时间（最快什么时候可以入职？）`
  ...
  technical: {
    count: "10-14",
    maxTokens: 3500,
    instructions: `Deep dive into ALL tech skills mentioned in the resume AND required by the JD. Cover:
...
Ask about SPECIFIC technologies named in the resume and JD, not generic questions.`
  ...
  project: {
    count: "7-9",
    maxTokens: 2500,
    instructions: `Drill into EACH significant project in the resume. For each project ask about:
- 项目背景与你的具体职责
- 遇到的最大技术挑战及解决方案
...
- 如果重来你会做哪些不同的设计决策`
  ...
  behavioral: {
    count: "5-7",
    maxTokens: 2000,
    instructions: `Situational/behavioral questions using STAR method. Cover:
- 团队冲突处理（与同事或上级意见相左时）
- 带领项目或团队的经历
- 在 deadline 压力下的表现
- 主动发现并解决问题的案例
...
- 从失败中学到的教训`
  ...
  system_design: {
    count: "2-4",
    maxTokens: 1500,
    instructions: `Design questions relevant to the tech stack and domain in the JD (only for mid-level or above roles):
...
- 缓存策略设计`
  ...
  open_ended: {
    count: "3-5",
    maxTokens: 1500,
    instructions: `Questions about growth mindset, culture fit, and motivation:
- 最近学习了哪些新技术？怎么学的？
- 你如何看待代码质量与交付速度之间的平衡？
- 工作之外有哪些技术社区贡献或个人项目？
...
export const INTERVIEW_CATEGORIES: CategoryKey[] = [
  "hr", "technical", "project", "behavioral", "system_design", "open_ended"
];

export function getCategoryMaxTokens(category: CategoryKey): number {
  return categoryConfigs[category].maxTokens;
}

export function buildCategoryInterviewPrompt(
  category: CategoryKey,
  resumeText: string,
  jobDescription: string
): string {
  const { count, instructions } = categoryConfigs[category];

  return `You are an expert technical interviewer. Generate exactly ${count} interview questions for the "${category}" category only.
...
- sampleAnswer: 3-6 sentences in Chinese, reference actual details from the resume
- keyPoints: 3-5 bullet points in Chinese
- Questions MUST reference specific project names, tech stack, companies, and metrics from the resume
- Do NOT generate generic placeholder questions
- Difficulty distribution: ~30% basic, ~45% intermediate, ~25% advanced
...
// Legacy: single-shot full prompt (kept for reference)
...
### "hr" — HR 面试官必问题（6-8 道）
${categoryConfigs.hr.instructions}

### "technical" — 技术基础（10-14 道）
${categoryConfigs.technical.instructions}

### "project" — 项目经历深挖（7-9 道）
${categoryConfigs.project.instructions}

### "behavioral" — 行为面试 STAR 法则（5-7 道）
${categoryConfigs.behavioral.instructions}

### "system_design" — 系统设计（2-4 道，适合中高级岗位）
${categoryConfigs.system_design.instructions}

### "open_ended" — 开放性问题（3-5 道）
${categoryConfigs.open_ended.instructions}
